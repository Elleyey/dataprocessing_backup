...
def extract_tvseries(dom):
    ...
    # make one list containing all the info off one serie
    superlist = []

    # iterate over series to gather information
    for info in series:
        ...
        # make a list containing the names as text, seperate by comma
        names = info.select('a[href*="/name/"]')
        nameslist = []
        for actor in names:
            nameslist.append(actor.get_text())
        name = ", ".join(nameslist)
        ...
        infolist = [title, rating, genre, name, runtime]
        superlist.append(infolist)
